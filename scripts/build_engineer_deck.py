...
def add_textbox(
    ...
    *,
    ...
    for i, line in enumerate(lines):
        ...
        _set_paragraph(
            p, line, size=size, color=color, bold=bold, align=align, font=font
        )
    return tb
...
def add_table(
    ...
    *,
    ...
    for c, h in enumerate(header):
        ...
        _set_paragraph(
            cell.text_frame.paragraphs[0], h, size=13, bold=True, color=WHITE
        )
    for r, row in enumerate(rows, start=1):
        for c, val in enumerate(row):
            ...
            cell.text_frame.clear()
            _set_paragraph(
                cell.text_frame.paragraphs[0], str(val), size=body_size, color=NAVY
            )
    return tbl
...
def slide_architecture(idx):
    ...
    items = [
        ...
        (
            "STAGING (DuckDB view):  stg_scb_bulkfil  ←  read_csv via httpfs, hive partitioning, latin-1",
            0,
        ),
        (
            "BRONZE-PARQUET (MinIO):  bronze_scb_bulkfil_parquet  ←  external mat, partition_by year/month/day",
            0,
        ),
        (
            "BRONZE-DUCKLAKE (MinIO + SQLite catalog):  bronze_scb_bulkfil_ducklake  ←  custom `ducklake` mat",
            0,
        ),
        (
            "SILVER LANDING ×2 (SQL Server):  scb_bulkfil_landing_from_{parquet,ducklake}  ←  custom `mssql_native` mat, with QUALIFY dedup",
            0,
        ),
        (
            "SILVER REJECTS (SQL Server):  scb_bulkfil_dedup_rejects  ←  same `mssql_native` mat, the QUALIFY-losers",
            0,
        ),
        (
            "SCD2 ×2 (SQL Server):  snap_scb_bulkfil_scd2{,_from_ducklake}  ←  dbt snapshot, --target sqlserver, strategy=check",
            0,
        ),
        ("", 0),
        (
            "Two custom materializations live in fidemo/macros/. Two `dbt run` targets (dev = DuckDB + extensions, sqlserver = native pyodbc).",
            0,
        ),
    ]
    add_bullet_list(s, left=0.7, top=1.3, width=12.0, height=5.7, items=items, size=14)
    add_footer(s, idx, TOTAL)
...
def slide_single_session(idx):
    ...
    add_title_bar(
        s,
        title="Single-session constraint",
    ...
    items = [
        ...
        (
            "The ducklake materialization runs `ATTACH 'ducklake:sqlite:...' AS lake` inside the model build.",
            0,
        ),
        (
            "Downstream `scb_bulkfil_landing_from_ducklake.sql` reads `lake.bronze.<name>` — that resolution requires `lake` to still be attached.",
            0,
        ),
        (
            "If we split the build into two `dbt run` calls (e.g. `make build-bronze` then `make load-landings`), the second invocation starts a fresh session, sees no `lake`, and the SELECT fails.",
            0,
        ),
        ("", 0),
        ("Mitigation in Makefile:", 0),
    ]
    add_bullet_list(s, left=0.7, top=1.3, width=12.0, height=2.6, items=items, size=14)
...
def slide_transactions(idx):
    ...
    add_title_bar(
        s,
        title="DuckLake writes silently dropped under default dbt-duckdb",
        subtitle="Empirical discovery: dbt reports OK, ducklake_snapshot_changes records nothing",
    )

    items = [
        (
            "Symptom: dbt run reported `OK created sql ducklake model …`. Re-querying `lake.bronze.<table>` showed no table. Probing the SQLite catalog directly: zero new entries in `ducklake_snapshot_changes`.",
            0,
        ),
        (
            "Cause: dbt-duckdb wraps every model invocation in BEGIN…COMMIT against the local DuckDB. The COMMIT does not propagate to attached non-DuckDB catalogs (DuckLake-over-SQLite in our case).",
            0,
        ),
        (
            "Without an explicit DuckLake-side commit, the writes vanish when the connection closes.",
            0,
        ),
        (
            "Fix: opt out of dbt-duckdb's transaction wrapping. Our materializations are single-statement CTAS; we don't need atomicity guarantees from dbt.",
            0,
        ),
    ]
    add_bullet_list(s, left=0.7, top=1.3, width=12.0, height=4.0, items=items, size=14)
...
def slide_encoding(idx):
    ...
    add_title_bar(
        s,
        title="DuckDB CSV encoding — `latin-1`, not ICU",
        subtitle="Most ICU encoding aliases mangle ASCII into fullwidth Unicode",
    )

    items = [
        (
            "Source files are Swedish (Latin-9 byte-content). DuckDB's CSV reader advertises 300+ ICU encoding names. Plausible-looking aliases include ISO8859_15, windows-1252, 8859_15.",
            0,
        ),
        (
            "All of those silently apply Unicode compatibility normalisation. ASCII `F` (U+0046) becomes fullwidth `Ｆ` (U+FF26) — column NAMES get rewritten, not just values.",
            0,
        ),
        ("Reliable encodings are limited to: utf-8, utf-16, latin-1.", 0),
        (
            "Latin-1 is functionally equivalent to ISO-8859-15 for our content (divergence is only on €, Š, œ, Ÿ — none appear in company names or addresses).",
            0,
        ),
    ]
    add_bullet_list(s, left=0.7, top=1.3, width=12.0, height=3.5, items=items, size=14)
...
def slide_scd2_dedup(idx):
    ...
    add_title_bar(
        s,
        title="SCD2 source contract: one row per business key",
        subtitle="Otherwise SQL Server's MERGE refuses with cardinality error 8672",
    )

    items = [
        (
            "Bronze legitimately has duplicates: each cross-delivery file may carry the same peorgnr at a new effective_date.",
            0,
        ),
        (
            "First snapshot run failed:  '[42000] The MERGE statement attempted to UPDATE or DELETE the same row more than once.'",
            0,
        ),
        (
            "Root cause: snapshot's MERGE matched one target row to N source rows (one per delivery) on `unique_key=peorgnr`. SQL Server refuses that cardinality.",
            0,
        ),
        (
            "Fix: dedup at silver, not bronze. Bronze keeps full history; silver landing emits one winning row per key (latest effective_date wins). DuckDB QUALIFY does this in one line.",
            0,
        ),
    ]
    add_bullet_list(s, left=0.7, top=1.3, width=12.0, height=3.5, items=items, size=14)
...
def slide_audit_pattern(idx):
    ...
    add_title_bar(
        s,
        title="Duplicate-row audit pattern",
...
def slide_pins(idx):
    ...
    add_table(
        ...
        rows=[
            ...
            ["dbt-duckdb == 1.10.1", "Tracks dbt-core 1.11.x; supports DuckDB 1.5.x"],
            [
...
def slide_dbt_show(idx):
    ...
    add_title_bar(
        s,
        title="`dbt show` against mssql_native is asymmetric",
        subtitle="Use `--target dev --select` for source-side, `--target sqlserver --inline + source()` for written-side",
    )

    items = [
        (
            "`mssql_native` is a DuckDB-only materialization (INSTALL + ATTACH + CTAS run in DuckDB).",
            0,
        ),
        (
            "`dbt show --target sqlserver --select <model>` asks the sqlserver adapter to re-execute the model's SELECT, which reads `ref('bronze_…')` — a Parquet location in MinIO. SQL Server can't reach Parquet → error.",
            0,
        ),
        (
            "All v2 silver tables are declared as sources in `_sources.yml` so `source('finance_landing','…')` reaches them from either target.",
            0,
        ),
    ]
    add_bullet_list(s, left=0.7, top=1.3, width=12.0, height=2.5, items=items, size=14)
...
def slide_followups(idx):
    s = prs.slides.add_slide(blank_layout)
    add_title_bar(s, title="Followups", subtitle="Concrete TODOs for the next sprint")
    items = [
        (
            "Wire the pipeline into a scheduled CI job (GitHub Actions, reusing the dev container image).",
            0,
        ),
        (
            "Move secrets from Makefile defaults to env-var injection or a real secrets manager (`.env` + `docker compose --env-file` is the minimum bar).",
            0,
        ),
        (
            "Decide one bronze format for production. DuckLake if compliance/time-travel matters; Parquet if max interop wins.",
            0,
        ),
        (
            "Migrate DuckLake catalog to PostgreSQL when the demo SQLite catalog can't keep up with concurrent writers.",
            0,
        ),
        (
            "Promote the `mssql_native` materialization from project-local Jinja macro to a shareable dbt-duckdb plugin (Python `BasePlugin` subclass).",
            0,
        ),
        (
            "Add SCD2 coverage for the second source domain (collateral / real-estate) using the same medallion pattern.",
            0,
        ),
        (
            "Replace the on-run-end dbt_artifacts upload with something more curated (or wire it into BI).",
            0,
        ),
    ]
    add_bullet_list(s, left=0.7, top=1.3, width=12.0, height=5.7, items=items, size=15)
    add_footer(s, idx, TOTAL)
...
slides = [
    ...
    slide_goals,
    slide_architecture,
    slide_materializations,
    slide_single_session,
    slide_transactions,
    slide_encoding,
    slide_scd2_dedup,
    slide_audit_pattern,
