...
- **DuckDB pinned `==1.5.1`.** Newest version where the `mssql` community extension is published for `osx_arm64`, `linux_amd64`, and `linux_arm64` (verified via HEAD-probe matrix against `community-extensions.duckdb.org` on 2026-04-14). 1.5.2+ and 1.6.x+ currently 404 across all three platforms. Re-probe before bumping.
